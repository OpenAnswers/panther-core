...
//        'connect timeout'           : this.getRetryTimeout(),
      ...
      if( this.isBuffering() ){
        var interval_id = setInterval( 
          function(){ self.flushBufferQueue(); }, 
          self.getBuffering()
        );
    ...
    addEventToQueue: function( ev, cb )
    {
      ...
      debug( "queue contents: ", function(){this.queue.join(',')} );
    ...
    validateAlertFields: function( ev ){
      ...
      if ( _.isEmpty(missing_columns) )
        return true;

      logger.error( 'Event is missing mandatory column(s): ['
            + missing_columns.join( ',' ) + '] ['+ _.keys(ev) +']')
      return false
