...
exports.rules = function( a, syslog_line )
{
  ...
  try
  {
    ...
    {
      case "named":
        if( $msg_match = $sys.message.match( /unexpected RCODE \((\w+)\) resolving (.*)\/(.*)\/(.*):(\S+) (\d+:\d+)/ ) )
        {
        ...
        else if( $msg_match = $sys.message.match( /^transfer of (\S+)\/IN from (\S+)#\d+: failed to connect: (.*) \d+:\d+$/ ) )
        {
          a.severity = 5;
      ...
      case "DtfService.exe":
        if( $msg_match = $sys.message.match( /(\w+)[\ ]+: (.*)/ ) )
        {
          switch( $msg_match[1] )
          {
            ...
            case "WARN": a.severity=4; break;
            case "INFO": a.severity=2; break;
