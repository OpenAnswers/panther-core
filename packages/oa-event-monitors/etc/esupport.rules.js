...
*/
  ...
  else if( $a = $subject.match( /^windows logs \((\d+)\)/ ) )
  {
    ...
    for( var l in $lines )
    {
      ...
      else if( $b = $lines[l].match( /The name "(.*)" could not be registered on the Interface with IP address (.*)./ ) )
      {
        a.node_alias = $b[2];
  ...
  {
    if( a.node.match( /(nef|NEF)/ ) )
      a.customer = "Bloomberg";
    else if( a.node.match( /^(oaxgs|OAXGS|xei)/ ) )
      a.customer = "XEROX";
    else if( a.node.match( /^(oa|OA|burton)/ ) )
      a.customer = "Open Answers";
    else if( a.node.match( /^gprod/ ) )
      a.customer = "SUNW";
    else if( a.node.match( /^(gatekeeper|CM)/ ) )
      a.customer = "RoyalC";
    else if( a.node.match( /^(ofsted|www\.safeguardingchildren\.org\.uk)/ ) )
      a.customer = "Ofsted";
    else if( a.node.match( /^libadu/ ) )
      a.customer = "Jimmo";
    else if( a.node.match( /^accounts-2/ ) )
      a.customer = "BDB Law";
    else if( a.node.match( /^(bl)/ ) )
      a.customer = "Camelot";
