...
module.exports = {
  ...
  index: function(req, res, next ){
    ...
    if( req.query.filter )
    {
      ...
      filterfinder = { name: 'fallback', user: null, system: true };
    ...
    Alerts.findOne( conditions, function( err, data )
    {
      ...
      if( req.body.owner && req.body.owner != '' )
      {
        ...
        if( data.owner && data.owner != '' )
          ...
          messages.push( 'owner set to [' + req.body.owner +']' );
      ...
      {
        if( req.body.acknowledged === 0 )
          messages.push( 'Un-Acknowledged' );
        else
          messages.push( 'Acknowledged' );
