...
- Migrated Coffeescript to TypeScript across the codebase
- Console frontend modernised
- Upgraded to Node 24
- Migrated build tooling from Yarn to npm
- Prettier applied across the codebase
- Removed old redundant scripts

## Fixes

- UUID missing during startup on some installs
- MongoDB `nModified` replaced with `modifiedCount`
- `apiKey` 404 fix
- Import panel fix
- Build fixes for minimatch/picomatch

## Security

- Fixed CVE-2024-32002
- Fixed GHSA-36fh-84j7-cv5h and GHSA-3h5v-q93c-6h6q
- Forced lodash to 4.17.21 for mincer (GHSA-35jh-r3h4-6jhm)
...
- `external_id` added as a default column
...
_NOTE_: CentOS 7 does not support RFC5424, if you have Docker available you can wrap the above command within a container
